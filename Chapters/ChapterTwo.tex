...
\label{sec:sota}
 ...
 \hspace{0.5cm} Atualmente, a \textit{IEEE Micromouse Competition} adota uma configuração que consiste em um labirinto de 16 x 16 blocos. Cada bloco possui 18 cm x 18 cm. As paredes, que possuem 5 cm de altura, são pintadas de branco de modo a ser reflexiva à luz infravermelha. O chão, por outro lado, é pintado de preto, para que não seja reflexivo. Além disso, os competidores sabem previamente que o \textit{micromouse} tem seu ponto de partida localizado em um dos cantos do labirintos, devendo alcançar o seu centro para terminar o desafio. Com base nisso, os participantes devem usar de algorítimos de busca para explorar o labirinto e encontrar a rota mais otimizada para o ponto de chegada estabelecido pela competição. O robô por sua vez, não pode ter suas dimensões maiores que uma seção de 25cm x 25 cm. As regras completas estão dispostas como anexo no final do documento.
...
\begin{table}[]
	\centering
	\caption{Especificações do Green Giant.}
	\label{tab:my-table}
	\begin{tabular}{|l|l|lll}
		\cline{1-2}
		\multicolumn{2}{|c|}{\cellcolor[HTML]{C0C0C0}\textbf{Green Giant 5.19V}} &  &  &  \\ \cline{1-2}
		Fabricante & Green Ye &  &  &  \\ \cline{1-2}
		\cellcolor[HTML]{EFEFEF}Ano & \cellcolor[HTML]{EFEFEF}2017 &  &  &  \\ \cline{1-2}
		Linguagem de programação & C/C++ &  &  &  \\ \cline{1-2}
		\cellcolor[HTML]{EFEFEF}Sensores & \cellcolor[HTML]{EFEFEF}IR, MPU, IE &  &  &  \\ \cline{1-2}
		Controlador & STM32 &  &  &  \\ \cline{1-2}
		\cellcolor[HTML]{EFEFEF}Simulador & \cellcolor[HTML]{EFEFEF}Não possui &  &  &  \\ \cline{1-2}
		Bateria & LiPo 300 mAh (7,4 V) &  &  &  \\ \cline{1-2}
		\cellcolor[HTML]{EFEFEF}Rodas & \cellcolor[HTML]{EFEFEF}3D printed mount\&wheel + mini-z tyres &  &  &  \\ \cline{1-2}
		Motor & DC-Motor 6 540 RPM 0,21 Nm (6 V) &  &  &  \\ \cline{1-2}
		\cellcolor[HTML]{EFEFEF}Interface de usuário & \cellcolor[HTML]{EFEFEF}DMD 5x7, LEDs, Botão, Bluetooth &  &  &  \\ \cline{1-2}
		Outros & Sistema de ventoinhas de sucção &  &  &  \\ \cline{1-2}
	\end{tabular}
\end{table}
